...
class play_MinMax_CombSA_new:
    ...
    def play(
        ...
        #     )
        # ) #変更
        INITIALIZATION_ROUND = int(np.floor(self.budget / self.N * self.beta))
        ...
        # Loop
        # Total_rounds = np.int(np.ceil(np.log2(self.K_actions))) #変更
        # B = 2 ** (np.ceil(np.log2(self.K_actions))) - 1 #変更
        Total_rounds = int(np.ceil(np.log2(self.N)))
        B = 2 ** (np.ceil(np.log2(self.N))) - 1
        ...
        for r in range(1, Total_rounds + 1):
            ...
            #     B / (2 ** (r - 1))
            # )  # 変更
            m_r = (T_dash - self.N * np.ceil(np.log2(self.N))) / (B / 2 ** (r - 1))
            n_r = None
            combination_list = []
            ...
            # ] #変更
            self.actions_list = self.actions_list[: int(np.ceil(self.N / (2**r)))]
